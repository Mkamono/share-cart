...
var UserWhere = struct {
	...
	ID:        whereHelperint{field: "\"main\".\"users\".\"id\""},
	Name:      whereHelperstring{field: "\"main\".\"users\".\"name\""},
	CreatedAt: whereHelpertime_Time{field: "\"main\".\"users\".\"created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"main\".\"users\".\"updated_at\""},
	DeletedAt: whereHelpernull_Time{field: "\"main\".\"users\".\"deleted_at\""},
}
...
type userR struct {
	AuthSubjects AuthSubjectSlice `boil:"AuthSubjects" json:"AuthSubjects" toml:"AuthSubjects" yaml:"AuthSubjects"`
}
...
func (r *userR) GetAuthSubjects() AuthSubjectSlice {
	if r == nil {
		return nil
	}
	return r.AuthSubjects
}
...
// AuthSubjects retrieves all the auth_subject's AuthSubjects with an executor.
func (o *User) AuthSubjects(mods ...qm.QueryMod) authSubjectQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"main\".\"auth_subjects\".\"user_id\"=?", o.ID),
	)

	return AuthSubjects(queryMods...)
}

// LoadAuthSubjects allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAuthSubjects(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`main.auth_subjects`),
		qm.WhereIn(`main.auth_subjects.user_id in ?`, argsSlice...),
		qmhelper.WhereIsNull(`main.auth_subjects.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load auth_subjects")
	}

	var resultSlice []*AuthSubject
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice auth_subjects")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on auth_subjects")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for auth_subjects")
	}
	...
	if singular {
		object.R.AuthSubjects = resultSlice
...
// AddAuthSubjects adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AuthSubjects.
// Sets related.R.User appropriately.
func (o *User) AddAuthSubjects(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*AuthSubject) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			AuthSubjects: related,
		}
	} else {
		o.R.AuthSubjects = append(o.R.AuthSubjects, related...)
	}
...
func Users(mods ...qm.QueryMod) userQuery {
	mods = append(mods, qm.From("\"main\".\"users\""), qmhelper.WhereIsNull("\"main\".\"users\".\"deleted_at\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"main\".\"users\".*"})
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"main\".\"users\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *User) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"main\".\"users\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"main\".\"users\" %sDEFAULT VALUES%s"
		}
...
func (o *User) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"main\".\"users\" SET %s WHERE %s",
...
func (o UserSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"main\".\"users\" SET %s WHERE %s",
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(userPrimaryKeyColumns) == 0 {
				return errors.New("boiler: unable to upsert users, could not build conflict column list")
			}

			conflict = make([]string, len(userPrimaryKeyColumns))
			copy(conflict, userPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"main\".\"users\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"main\".\"users\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o UserSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"main\".\"users\" WHERE " +
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"main\".\"users\" SET %s WHERE "+
...
func (o *UserSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"main\".\"users\".* FROM \"main\".\"users\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, userPrimaryKeyColumns, len(*o)) +
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"main\".\"users\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
